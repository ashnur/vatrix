void function(root){
    ...
    function matrix_pivotize(matrix, P){
        var max = indexof_max(matrix_column(matrix, 0));
        if ( max !== 0 ) {
            interchange(matrix,0,max)
            interchange(P,0,max)
        }
    }

    function matrix_ludecomposition(matrix){
        var P=identity_matrix(matrix.length), L, U;
        (matrix[0].length).times(function(column){
            matrix_pivotize(matrix, P)
