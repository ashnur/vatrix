void function(root){
    ...
    function vector_gcd(vector){ return vector.reduce(gcd) }

    function matrix_gcd(matrix){
        return vector_gcd(matrix.map(vector_gcd))
    }
    ...
    function add(p,c){ p=p||0; c=c||0; return Number(p)+Number(c) }

    function subtract(p,c){ p=p||0; c=c||0; return Number(p)-Number(c) }

    function multiply(p,c){ p=p||0; c=c||0; return Number(p)*Number(c) }

    function divide(p,c){ p=p||0; c=c||0; return Number(p)/Number(c) }
    ...
    function matrix_scalar_division(matrix, scalar){
        return matrix.map(function(vector){
            return scalar_division(vector, scalar)
        })
    }
    ...
    function matrix_echelon(matrix){
        ...
        var width = matrix[0].length
            ...
            , echelon = matrix_multiplication(augmentation, matrix)
            ;
        ...
        echelon.forEach(function(row, row_index){
            times(row_index, function(column_index){
                ...
                multiple = lcm(pivot, lead)

                if ( multiple ) {
                    echelon[column_index] = scalar_multiplication(echelon[column_index], multiple/pivot)
                    augmentation[column_index] = scalar_multiplication(augmentation[column_index], multiple/pivot)

                    echelon[row_index] = scalar_multiplication(echelon[row_index], multiple/lead)
                    augmentation[row_index] = scalar_multiplication(augmentation[row_index], multiple/lead)

                    change *= (multiple/pivot * multiple/lead)

                    pivot_row = echelon[column_index]
                    echelon[row_index] = vector_subtraction(echelon[row_index], pivot_row)
                    augmentation[row_index] = vector_subtraction(augmentation[row_index], pivot_row)
                }
            })
        })
    ...
    function matrix_fully_reduce(matrix){
        var echelon_result = matrix_echelon(matrix)
            ...
            console.log(drawMatrix(echelon))
        echelon.forEach(function(row, row_index){
