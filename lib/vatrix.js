void function(root){
    ...
    function matrix_pivotize(matrix){
        ...
        times(size, function(row_index){
            if ( matrix[row_index][row_index] === 0 ) {
                times(size, function(search){
                    if ( search !== row_index
    ...
    function matrix_echelon(matrix){
        ...
        var width = matrix[0].length
            ...
            , change = r(1).times(pivotize_result[1])
    ...
    function matrix_determinant(matrix){
        ...
        }).reduce(function(p, c, i ){ return i === 0 ? c : p.times(c) }).times(echelon[1]).val()

    }

    function matrix_fully_reduce(echelon, augmentation){
    }
