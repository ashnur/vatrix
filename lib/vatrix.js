void function(root){
    ...
    function matrix_rationalize(matrix){
        return matrix.map(function(row){
            return row.map(function(elem){ return r(elem) })
        })
    }
        ...
        echelon.forEach(function(row, row_index){
            row_index.times(function(column_index){
                var pivot = echelon[column_index][column_index]
                    ...
                    , l = r(U[row_index][column_index], pivot)
                    ;

                U[row_index] = vector_subtraction(U[row_index], scalar_multiplication(pivot_row, l))
            })
        })
        return [change, echelon]
    }
    ...
    function matrix_determinant(matrix){
        ...
        var plu = matrix_ludecomposition(matrix);
        return matrix_multiplication(plu[1],plu[2])
